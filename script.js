...
contactForm.addEventListener('submit', async (e) => {
    ...
    btn.addEventListener('click', () => {
      ...
      projectCards.forEach(card => {
        ...
        if (filterValue === 'all' || category === filterValue) {
          ...
          // Re-trigger animasi fade-in jika perlu (opsional)
  ...
  // 4. Testimonial Slider sederhana
  ...
  // 5. Progress bar animasi saat muncul di viewport
  const progressBars = document.querySelectorAll('.progress-fill');
  const progressObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // progress bar sudah memiliki lebar dari inline style, biarkan saja
        // Bisa juga ditambahkan animasi transisi
